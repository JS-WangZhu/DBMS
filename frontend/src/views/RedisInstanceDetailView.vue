  ...
  <div class="performance-page">
    <el-card class="toolbar-card" shadow="never">
      <div class="toolbar-header">
        <div>
          <div class="page-title">Redis 实例详情</div>
      ...
      <div class="filter-grid">
        ...
        <div class="filter-item filter-item--instance">
          ...
          <el-select v-model="instanceId" filterable placeholder="请选择 Redis 实例" @change="loadPerformance">
            <el-option
    ...
    <section class="section-block" v-loading="loading">
      <div class="section-heading">
        <div><h2>数据库运行指标</h2><span>实例连接数与实例内存使用率趋势</span></div>
      </div>
...
const databaseCharts = [
  { key: "connections", title: "实例连接数", fields: [{ key: "connections_current", name: "当前连接", color: "#409eff" }], unit: "" },
  { key: "redisMemory", title: "实例内存使用率", fields: [{ key: "redis_memory_usage_pct", name: "内存使用率", color: "#22a06b" }], unit: "%" },
];
...
async function syncInstanceSelection() {
  ...
  if (!available.some((item) => item.id === instanceId.value)) instanceId.value = available[0]?.id || null;
...
function chartOption(chart) {
  ...
  return {
    ...
    xAxis: { type: "time", axisLabel: { color: "#909399", hideOverlap: true }, splitLine: { show: false } },
    yAxis: {
      type: "value",
      min: 0,
...
async function loadInstances() {
  try {
    const response = await listInstances("redis", { action: "view_instance" });
    instances.value = response.data?.data || [];
    if (!instanceId.value && instances.value.length) instanceId.value = instances.value[0].id;
  } catch (error) {
    ElMessage.error(error.response?.data?.message || "Redis 实例加载失败");
  }
}

async function loadClusters() {
  try {
    const response = await listClusters("redis", { action: "view_instance" });
    clusters.value = response.data?.data || [];
  } catch (error) {
    clusters.value = [];
    ElMessage.error(error.response?.data?.message || "Redis 集群加载失败");
  }
}
...
.basic-grid, .database-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
