  ...
  <el-card>
    ...
    <el-table :data="pagedRows" v-loading="loading" stripe>
      ...
      <el-table-column label="模式" min-width="100">
        <template #default="scope">{{ scope.row.latest?.summary?.mode || scope.row.cluster_mode || "-" }}</template>
      </el-table-column>
      <el-table-column label="集群状态" width="110">
        <template #default="scope">
          <el-tag v-if="scope.row.latest?.summary?.cluster_state === 'ok'" type="success">ok</el-tag>
          <el-tag v-else-if="scope.row.latest?.summary?.cluster_state" type="danger">{{ scope.row.latest?.summary?.cluster_state }}</el-tag>
          <span v-else>-</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="主库" width="90">
        <template #default="scope">
          <el-tag type="danger">{{ scope.row.latest?.summary?.master_count ?? "-" }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="从库" width="90">
        <template #default="scope">
          <el-tag type="success">{{ scope.row.latest?.summary?.slave_count ?? "-" }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="probeDialogVisible" title="Redis 集群连接性探测" width="1080px" top="5vh">
      <div v-loading="probing">
        <el-descriptions :column="4" border>
          ...
          <el-descriptions-item label="集群状态">
            <el-tag v-if="probeResult?.summary?.cluster_state === 'ok'" type="success">ok</el-tag>
            <el-tag v-else-if="probeResult?.summary?.cluster_state" type="danger">{{ probeResult?.summary?.cluster_state }}</el-tag>
            <span v-else>-</span>
          </el-descriptions-item>
          <el-descriptions-item label="主库">
            <el-tag type="danger">{{ probeResult?.summary?.master_count ?? "-" }}</el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="从库">
            <el-tag type="success">{{ probeResult?.summary?.slave_count ?? "-" }}</el-tag>
          </el-descriptions-item>
        ...
        <el-table :data="probeResult?.nodes || []" stripe class="mt-16">
          ...
          <el-table-column label="复制源" min-width="180">
            <template #default="scope">{{ scope.row.replication_source || (scope.row.master_host ? `${scope.row.master_host}:${scope.row.master_port || ""}` : "-") }}</template>
          </el-table-column>
          <el-table-column label="复制链路" width="120">
            <template #default="scope">
              <el-tag v-if="scope.row.master_link_status === 'up'" type="success">up</el-tag>
              <el-tag v-else-if="scope.row.master_link_status" type="danger">{{ scope.row.master_link_status }}</el-tag>
              <span v-else>-</span>
            </template>
          </el-table-column>
          <el-table-column label="从库数" width="90">
            <template #default="scope">{{ scope.row.connected_slaves ?? "-" }}</template>
          </el-table-column>
          <el-table-column label="集群模式" width="110">
            <template #default="scope">
              <el-tag v-if="scope.row.cluster_enabled === '1' || scope.row.cluster_enabled === 1" type="warning">启用</el-tag>
              <span v-else>-</span>
            </template>
          </el-table-column>
    ...
    <el-dialog v-model="historyDialogVisible" title="Redis 拓扑变更历史" width="1080px" top="5vh">
      <div v-loading="historyLoading">
        <el-table :data="historyData.items" stripe>
          ...
          <el-table-column label="模式" width="100">
            <template #default="scope">{{ scope.row.topology?.mode || "-" }}</template>
          </el-table-column>
          <el-table-column label="集群状态" width="110">
            <template #default="scope">
              <el-tag v-if="scope.row.topology?.cluster_state === 'ok'" type="success">ok</el-tag>
              <el-tag v-else-if="scope.row.topology?.cluster_state" type="danger">{{ scope.row.topology?.cluster_state }}</el-tag>
              <span v-else>-</span>
            </template>
          </el-table-column>
          <el-table-column label="总节点" width="90">
            <template #default="scope">{{ scope.row.topology?.total_nodes ?? "-" }}</template>
          </el-table-column>
          <el-table-column label="M / S" width="100">
            <template #default="scope">
              {{ scope.row.topology?.master_count ?? 0 }} / {{ scope.row.topology?.slave_count ?? 0 }}
            </template>
          </el-table-column>
          <el-table-column label="节点" min-width="360">
            <template #default="scope">
              <el-tag
                v-for="n in scope.row.topology?.nodes || []"
                :key="n.host"
                :type="roleTagType(n.role)"
                class="member-tag"
              >
                {{ n.host }} [{{ n.role }}]<span v-if="n.master_host"> ← {{ n.master_host }}</span>
              </el-tag>
            </template>
          </el-table-column>
...
const displayRows = computed(() => {
  ...
  return rows.value.filter((row) => {
    ...
    if (!kw) return true;
    const text = `${row.business_line || row.namespace || ""} ${row.environment || ""} ${row.name || ""}`.toLowerCase();
...
function roleTagType(role) {
  ...
  if (text === "master") return "danger";
  if (text === "slave" || text === "replica") return "success";
  return "info";
}
...
async function loadData() {
  loading.value = true;
  try {
    const { data } = await listClusters("redis");
