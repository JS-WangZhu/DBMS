  ...
  <el-card>
    <template #header>
      <div class="header-row">
        <span>Doris 定制管理</span>
        <div>
          <el-button type="primary" @click="dialogVisible = true">新增Doris实例</el-button>
    ...
    <el-table :data="rows" v-loading="loading" stripe>
      ...
      <el-table-column label="定制操作" width="180">
        <template #default="scope">
          <el-button link type="primary" @click="viewFeStatus(scope.row)">FE状态</el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog v-model="dialogVisible" title="新增Doris实例" width="560px">
...
const rows = ref([]);
const form = reactive({ name: "", host_input: "", port: 9030, username: "", password: "" });

async function loadList() {
  loading.value = true;
  try {
    const { data } = await listDorisInstances();
...
async function onCreate() {
  saving.value = true;
  try {
    await createDorisInstance({ ...form });
...
async function viewFeStatus(row) {
  try {
    const { data } = await dorisFeStatus(row.id);
    await ElMessageBox.alert(`<pre style='white-space:pre-wrap'>${JSON.stringify(data.data, null, 2)}</pre>`, "FE状态", {
