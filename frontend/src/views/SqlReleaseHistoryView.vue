  ...
  <div class="page">
    <el-card>
      ...
      <el-table :data="rows" stripe v-loading="loading">
        ...
        <el-table-column label="状态" width="135"><template #default="scope"><el-tag :type="statusType(scope.row.status)">{{ statusLabel(scope.row.status) }}</el-tag></template></el-table-column>
        <el-table-column prop="created_at" label="提交时间" width="180" />
        <el-table-column label="操作" width="350" fixed="right">
          <template #default="scope">
            <el-button link type="primary" @click="openDetail(scope.row)">详情</el-button>
            <el-button link type="primary" @click="openShare(scope.row)">分享</el-button>
            <el-button v-if="scope.row.can_skip_review" link type="warning" :loading="skippingId === scope.row.id" @click="skipReview(scope.row)">跳过审核</el-button>
            <el-button link type="primary" @click="resubmit(scope.row)">再次提交</el-button>
            <el-button v-if="scope.row.can_execute && canExecute(scope.row)" link type="danger" :loading="executingId === scope.row.id" @click="execute(scope.row)">{{ scope.row.backup_confirmation_required ? '跳过备份并继续' : scope.row.status === 'failed' ? '重试' : '执行' }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-drawer v-model="drawer" title="上线详情" size="78%" @closed="stopDetailPolling">
      <template v-if="current"><div v-loading="detailLoading">
        ...
        <el-alert
          v-if="current.backup_confirmation_required"
          :title="`第 ${current.backup_failed_line} 条语句备份失败，执行已暂停；可确认跳过该语句备份后继续。`"
          type="warning"
          show-icon
          :closable="false"
        ...
        >
          <template #default>
            <el-button type="warning" size="small" :loading="executingId === current.id" @click="execute(current)">跳过备份并继续</el-button>
          </template>
        </el-alert>
    ...
    <el-dialog v-model="shareDialogVisible" title="分享工单" width="min(620px, 94vw)" destroy-on-close>
      <SqlReleaseShareCard v-if="shareRelease" :release="shareRelease" />
    </el-dialog>
  </div>
</template>
...
import { executeSqlRelease, getSqlRelease, listSqlReleases, rollbackSqlRelease, skipSqlReleaseReview } from "../api/modules/sqlReleases";
...
const rows = ref([]), loading = ref(false), page = ref(1), pageSize = ref(10), total = ref(0), drawer = ref(false), current = ref(null), executingId = ref(null), skippingId = ref(null);
const detailLoading = ref(false), rollbackLoading = ref(false), selectedRollbackLines = ref([]);
const shareDialogVisible = ref(false), shareRelease = ref(null);
...
const statusOptions = [
  ...
  { label: "备份失败待确认", value: "backup_failed" },
...
const statusLabel = (value) => ({ reviewing: "初审中", review_rejected: "初审未通过", review_failed: "初审失败", backup_failed: "备份失败待确认", pending: "待执行", executing: "执行中", success: "成功", failed: "部分失败", rolling_back: "回滚中", rolled_back: "已回滚", partial_rolled_back: "部分回滚", rollback_failed: "回滚失败" }[value] || value);
const statusType = (value) => ({ reviewing: "info", review_rejected: "warning", review_failed: "danger", backup_failed: "warning", pending: "warning", executing: "primary", success: "success", failed: "danger", rolling_back: "primary", rolled_back: "success", partial_rolled_back: "warning", rollback_failed: "danger" }[value] || "info");
const reviewLabel = (row) => row.review_skipped ? "已跳过" : row.status === "reviewing" ? "审核中" : row.status === "review_failed" ? "失败" : row.ai_passed ? "通过" : "未通过";
const reviewType = (row) => row.review_skipped ? "warning" : row.status === "reviewing" ? "info" : row.status === "review_failed" ? "danger" : row.ai_passed ? "success" : "warning";
...
const backupLabel = (row) => row.has_rollback ? `已备份 ${row.backup_rows || 0} 行` : row.backup_status === "skipped_by_operator" ? "已确认跳过" : row.backup_status === "skipped" || row.status === "backup_skipped" ? "非 DML 不备份" : ["backing_up"].includes(row.status) ? "生成中" : "未生成";
...
function openShare(row) { shareRelease.value = row; shareDialogVisible.value = true; }
async function skipReview(row) {
  try {
    await ElMessageBox.confirm("AI 模型预审出现异常。跳过后工单将直接进入待执行，且不会产生 AI 风险结论。确认跳过本次审核？", "跳过 AI 预审确认", { type: "warning", confirmButtonText: "确认跳过" });
  } catch {
    return;
  }
  skippingId.value = row.id;
  try {
    const { data } = await skipSqlReleaseReview(row.id);
    ElMessage.success(data.message || "已跳过 AI 预审");
    if (current.value?.id === row.id) current.value = data.data;
    await loadRows();
  } catch (error) {
...
async function executeWithBackupConfirmation(row, initialPayload) {
  let payload = initialPayload;
  while (true) {
    try {
      return await executeSqlRelease(row.id, payload);
    } catch (error) {
      const detail = error.response?.data?.data;
      if (error.response?.status !== 409 || !detail?.backup_confirmation_required || !detail?.backup_failed_line) throw error;
      try {
        await ElMessageBox.confirm(
          `第 ${detail.backup_failed_line} 条语句备份失败：${detail.execution_result?.error || "无法生成回滚备份"}。跳过后该语句无法通过工单自动回滚，是否继续执行？`,
          "备份失败确认",
          { type: "warning", confirmButtonText: "跳过备份并继续", cancelButtonText: "暂停执行", distinguishCancelAndClose: true },
        );
      } catch {
        return null;
      }
      payload = { confirm_skip_backup: true, skip_backup_line: detail.backup_failed_line };
    }
  }
}

async function execute(row) {
  const rejected = row.status === "review_rejected";
  if (!row.backup_confirmation_required) {
    const mongoNotice = row.db_type === "mongodb" ? "MongoDB 当前按语句逐条执行，但暂不生成回滚备份。" : row.execution_mode === "agent" ? "本工单将由实例绑定的 Agent 逐条提交；Agent 通道当前不生成回滚备份。" : "每条 SQL 都会先生成并保存回滚语句，再单独提交执行。";
    const message = rejected ? `AI 初审未通过。确认接受审核提示的风险并继续执行？${mongoNotice}` : `执行后会直接变更数据库；${mongoNotice}确认执行？`;
    try {
      await ElMessageBox.confirm(message, rejected ? "风险确认" : "执行确认", { type: "warning", confirmButtonText: rejected ? "确认风险并执行" : "执行" });
    } catch {
      return;
    }
  }
  ...
  try {
    const response = await executeWithBackupConfirmation(row, { confirm_risk: rejected });
    if (response) ElMessage.success("执行成功");
    await loadRows();
    if (current.value?.id === row.id) await loadDetail(row.id, true);
  } catch (error) {
...
onMounted(async () => {
  await loadRows();
  const sharedReleaseId = Number(route.query.release_id);
  if (Number.isInteger(sharedReleaseId) && sharedReleaseId > 0) {
    openDetail({ id: sharedReleaseId });
  }
});
