<template>
  <div class="page apply-page">
        ...
        <div class="architecture-panel">
          <div class="architecture-box">
            <div class="architecture-title"><div class="architecture-icon"><el-icon><Share /></el-icon></div>
              <div><strong>{{ selectedArchitecture.label }}</strong><span>{{ selectedArchitecture.tip }}</span></div>
            </div><el-tag type="success" effect="light">{{ selectedArchitecture.tag }}</el-tag>
          </div>
          <div v-if="architectureOptions.length > 1" class="architecture-options">
            <button
              v-for="option in architectureOptions"
              :key="option.key"
              type="button"
              class="architecture-option"
              :class="{ active: form.architectureKey === option.key }"
              @click="selectArchitecture(option)"
            >
              <span class="architecture-option-main">
                <strong>{{ option.label }}</strong>
                <em>{{ option.tip }}</em>
              </span>
              <el-icon v-if="form.architectureKey === option.key"><Check /></el-icon>
            </button>
          </div>
        </div>
      </section>
      ...
      <section class="config-row">
        ...
        <div class="storage-panel">
          ...
          <div class="storage-custom">
            <div class="storage-custom-label">
              <strong>自定义档位</strong>
              <span>支持直接输入 GB 数值</span>
            </div>
            <el-input-number v-model="form.storageGb" :min="10" :max="5000" :step="10" controls-position="right" class="storage-input" />
    ...
    <div class="submit-bar">
      <div class="selection-summary"><span>当前选择</span><strong>{{ selectedEngine.name }} {{ selectedEngine.version }}</strong><i></i><span>{{ selectedSpecification.cpu }}C{{ selectedSpecification.memory }}G / {{ form.storageGb }}GB</span><i></i><span>{{ selectedArchitecture.label }}</span></div>
      <el-button type="primary" size="large" :loading="submitting" @click="handleNext">下一步：申请预览 <el-icon class="el-icon--right"><ArrowRight /></el-icon></el-button>
    </div>
  </div>
</template>
...
const engines = [
  ...
  { key:"mongodb", badge:"M", name:"MongoDB", version:"7.0", description:"文档型数据库", architecture:"副本集模式", architectureTip:"主从自动选举，保障数据冗余与服务连续性", architectureTag:"高可用", architectureOptions:[{ key:"replica_set", label:"副本集模式", tip:"主从自动选举，保障数据冗余与服务连续性", tag:"高可用" }, { key:"sharded_cluster", label:"分片集群模式", tip:"按分片横向扩展，适合海量数据与高并发场景", tag:"可扩展" }] },
  ...
  { key:"postgresql", badge:"Pg", name:"PostgreSQL", version:"18", description:"高级开源关系型数据库", architecture:"主备高可用模式", architectureTip:"基于流复制构建主备架构，支持故障切换" },
];
...
const form=reactive({ region:"cn-shanghai", engine:"mysql", version:"8.4", architecture:"InnoDB Cluster 高可用模式", architectureKey:"default", specification:"small", cpu:2, memory:4, storageGb:100, name:"", environment:"production", reason:"" });
