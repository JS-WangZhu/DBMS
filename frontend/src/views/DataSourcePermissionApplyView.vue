    ...
    <el-card shadow="never">
      ...
      <el-tabs v-model="activeTab">
        <el-tab-pane v-if="!isAdmin" label="申请权限" name="apply">
          ...
          <el-table :data="filteredClusters" stripe row-key="id" empty-text="未找到生产环境数据源">
            ...
            <el-table-column label="申请查询" width="130" align="center"><template #default="{ row }"><div class="permission-switch"><el-switch :model-value="permissionSwitchValue(row.id, 'can_query')" :disabled="hasPermission(row.id, 'can_query')" @change="setPermission(row, 'can_query', $event)" /><span v-if="hasPermission(row.id, 'can_query')">已拥有</span></div></template></el-table-column>
            <el-table-column label="申请变更" width="130" align="center"><template #default="{ row }"><div class="permission-switch"><el-switch :model-value="permissionSwitchValue(row.id, 'can_change')" :disabled="hasPermission(row.id, 'can_change')" @change="setPermission(row, 'can_change', $event)" /><span v-if="hasPermission(row.id, 'can_change')">已拥有</span></div></template></el-table-column>
...
const effective = ref({}), selection = reactive({}), reason = ref("");
...
async function submitApplication() { if (!reason.value.trim()) return ElMessage.warning("请填写申请原因"); submitting.value = true; try { await createPermissionApplication({ reason: reason.value, items: selectedItems.value }); ElMessage.success("权限申请已提交"); Object.keys(selection).forEach((key) => delete selection[key]); reason.value = ""; activeTab.value = "history"; await loadApplications(); } finally { submitting.value = false; } }
...
.permission-switch { display: inline-flex; align-items: center; gap: 6px; }
.permission-switch span { color: var(--el-color-success); font-size: 12px; white-space: nowrap; }
