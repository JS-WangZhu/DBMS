...
async function startProbe() { if (!instanceId.value || starting.value) return; starting.value = true; clearState(); try { const { data: response } = await startMongoSessionProbe(instanceId.value); const data = response.data || {}; probeToken.value = data.token || ""; expiresAt.value = data.expires_at || null; canKill.value = Boolean(data.can_kill); updateCountdown(); countdownTimer = window.setInterval(updateCountdown, 1000); pollTimer = window.setInterval(fetchSessions, 3000); await fetchSessions(); ElMessage.success("会话抓取已开始"); } catch (error) { clearState(); ElMessage.error(error.response?.data?.message || "启动会话探测失败"); } finally { starting.value = false; } }
async function fetchSessions(force = false) { const token = probeToken.value; if (!token) return; if (fetching.value) { if (force) refreshQueued = true; return; } fetching.value = true; try { const { data: response } = await getMongoOperations(token); const data = response.data || {}; if (token !== probeToken.value) return; const next = data.sessions || []; const ids = new Set(next.map((item) => item.id)); hiddenIds.value = new Set([...hiddenIds.value].filter((id) => ids.has(id))); sessions.value = next; collectedAt.value = data.collected_at || null; expiresAt.value = data.expires_at || expiresAt.value; } catch (error) { if (token !== probeToken.value) return; if ([403, 410, 502].includes(error.response?.status)) clearState(); ElMessage.error(error.response?.data?.message || "抓取 currentOp 失败"); } finally { fetching.value = false; if (refreshQueued && probeToken.value) { refreshQueued = false; window.setTimeout(fetchSessions, 0); } } }
async function stopProbe(showMessage = false) { const token = probeToken.value; if (!token || stopping.value) return; stopping.value = true; clearState(true); try { await stopMongoSessionProbe(token); if (showMessage) ElMessage.success("会话探测连接已关闭，已保留当前输出"); } catch (error) { if (showMessage && error.response?.status !== 410) ElMessage.warning(error.response?.data?.message || "连接已在服务端关闭"); } finally { stopping.value = false; } }
...
onMounted(async () => { window.addEventListener("pagehide", closeOnPageHide); try { const [clusterResponse, instanceResponse] = await Promise.all([listClusters("mongodb", { action: "query" }), listInstances("mongodb", { action: "query" })]); clusters.value = clusterResponse.data?.data || []; instances.value = instanceResponse.data?.data || []; syncInstance(); } catch (error) { ElMessage.error(error.response?.data?.message || "MongoDB 实例与集群加载失败"); } });
