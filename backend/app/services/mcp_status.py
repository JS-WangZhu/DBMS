...
def _metrics(payload, db_type):
    keys = COMMON_METRIC_KEYS + DB_METRIC_KEYS.get(db_type, ())
...
def _alerts(instance, payload, snapshot, status, age):
    ...
    lag = _to_float(_pick(payload, "seconds_behind_master", "replication_lag_seconds", "repl_lag_seconds"))
...
def build_mcp_instance_status(filters=None, allowed_cluster_ids=None):
    ...
    for instance in instances:
        ...
        row = {
            ...
            "status": status,
            "collector_ok": payload.get("ok"),
            "ping_ok": payload.get("ping_ok"),
            "collector_error": payload.get("error") or None,
            "probe_source": payload.get("probe_source") or None,
            "payload_collected_at": payload.get("collected_at") or None,
            "version": _version(instance, payload),
            "cluster": _cluster_data(clusters.get(instance.cluster_id)),
            "metrics": _metrics(payload, instance.db_type),
            "alerts": alerts,
